...
def arming():
	...
	try:
		arming_cl = rospy.ServiceProxy("/uav3/mavros/cmd/arming", CommandBool)
...
def takeoff():
	mode = rospy.ServiceProxy("/uav3/mavros/set_mode", SetMode)
...
if __name__=="__main__":
	...
	# mavros.set_namespace("/uav3/mavros")
	pub = rospy.Publisher("/uav3/mavros/setpoint_velocity/cmd_vel_unstamped",Twist,queue_size=100)
	pub1 = rospy.Publisher("/uav3/mavros/setpoint_raw/local",PositionTarget,queue_size=100)
	local_pos_pub = rospy.Publisher("/uav3/mavros/setpoint_position/local", PoseStamped, queue_size=100)
	state_sub = rospy.Subscriber("/uav3/mavros/state", State, state_cb)
	uav_pos=rospy.Subscriber("/uav3/mavros/global_position/local",Odometry,handle_poses)
	...
	rospy.Subscriber("chatter3", Twist, callback)
	rospy.Subscriber("setpoint_pos3", PoseStamped, pos_callback)
	rospy.init_node('teleop_twist_keyboard3')
